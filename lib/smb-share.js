...
class SMBShare {
  ...
  disconnect () {
    ...
    this.closeAllFiles()
    if (!this._session.isNull() && this._tid) {
  ...
  getFileStat (path) {
    ...
    const stat = Libdsm.smb_fstat(this._session, this._tid, path)
    const _fullPath = `\\${this._shareName}\\${path}`

    if (!stat) {
      const description = `An error has occurred while reading file stat at '\\\\${this._parent.server}${_fullPath}'.`
      debugLog(description)
      throw new Error(description)
    }

    const retVal = {
      size: Libdsm.smb_stat_get(stat, SMBStatTypes.SMB_STAT_SIZE),
      directory: !!Libdsm.smb_stat_get(stat, SMBStatTypes.SMB_STAT_ISDIR),
      created: BigInt(Libdsm.smb_stat_get(stat, SMBStatTypes.SMB_STAT_CTIME)),
      accessed: BigInt(Libdsm.smb_stat_get(stat, SMBStatTypes.SMB_STAT_ATIME)),
      written: BigInt(Libdsm.smb_stat_get(stat, SMBStatTypes.SMB_STAT_WTIME)),
      changed: BigInt(Libdsm.smb_stat_get(stat, SMBStatTypes.SMB_STAT_MTIME))
    }
